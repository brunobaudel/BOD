...
public class CriarBD extends SQLiteOpenHelper {
	...
	private CriarBD(Context context) {
		super(context, "BD_BUS_OPENDOCS", null, 1);
	}
	...
	public void onCreate(SQLiteDatabase db) {
		for (int i = 0; i <  TabelasBusOpenDocs.getTables().length; i++) {
			db.execSQL( TabelasBusOpenDocs.getTables()[i]);
			if(TabelasBusOpenDocs.getTables()[i].contains("tbl_log_atualizacao")){
				for (int j = 0; j <  TabelasBusOpenDocs.getTables().length; j++){
